  ...
  updateQuery(query, minQty, minRating, tags, callback) {
    ...
    if (tags != '') {
      this.connection.query(
        ...
        minQty +
        ' AND Tag.value IN (' +
        this.connection.escape(tags) +
        ') GROUP BY Product.id' +
        ' HAVING COUNT(distinct Tag.Id) = ' +
        tags.length +
        ' AND ROUND(AVG(rating), 1) >= ' + minRating,
      ...
      console.log('SELECT Product.id, growerId, name, price, quantity, imageURL, ROUND(AVG(rating), 1) AS AvgRating' +
        ' FROM Product INNER JOIN ProductReview ON Product.id = ProductReview.productID' +
        ' WHERE (name LIKE ' +
        this.connection.escape('%' + query + '%') +
        ' OR Product.description LIKE ' +
        ...
        minQty +
        ' GROUP BY Product.id' +
        ' HAVING ROUND(AVG(rating), 1) >= ' + minRating)
      this.connection.query(
        'SELECT Product.id, growerId, name, price, quantity, imageURL, ROUND(AVG(rating), 1) AS AvgRating' +
        ' FROM Product INNER JOIN ProductReview ON Product.id = ProductReview.productID' +
        ' WHERE (name LIKE ' +
